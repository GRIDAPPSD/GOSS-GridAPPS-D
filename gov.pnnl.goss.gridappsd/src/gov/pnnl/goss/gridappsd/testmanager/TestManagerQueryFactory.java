...
public class TestManagerQueryFactory {
    ...
    public String getGeographicalRegion() {
        ...
        String geoRegionQuery = "SELECT ?name WHERE { " + "?s r:type c:GeographicalRegion. "
    ...
    public String getSubGeographicalRegion() {
        QueryHandler queryHandler = getQueryHandler();
        String subGeoRegionQuery = "SELECT ?name WHERE { " + "?s r:type c:SubGeographicalRegion. "
                + "?s c:IdentifiedObject.name ?name} "
                + "ORDER by ?name";
    ...
    public QueryHandler getQueryHandler() {
        ...
        // + "PREFIX c: <http://iec.ch/TC57/2012/CIM-schema-cim16#> " + "SELECT ?name
        // WHERE { "
        // + "?s r:type c:Line. " + "?s c:IdentifiedObject.name ?name} " + "ORDER by
        // ?name";

        // String bgHost =
        // configurationManager.getConfigurationProperty(GridAppsDConstants.BLAZEGRAPH_HOST_PATH);
        // if (bgHost == null || bgHost.trim().length() == 0) {
        // bgHost = "http://localhost:9999";
        // }
        String bgHost = "http://localhost:8889";
        QueryHandler queryHandler = new BlazegraphQueryHandler(bgHost + "/bigdata/namespace/kb/sparql", null, "1",
                "manager");

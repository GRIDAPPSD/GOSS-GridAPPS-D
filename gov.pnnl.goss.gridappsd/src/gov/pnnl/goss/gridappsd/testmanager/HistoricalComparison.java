...
public class HistoricalComparison {
    ...
    // public TestResultSeries test_proven_one_at_a_time(String simulationId,
    // JsonObject expected_output_series){
    ...
    //// responseStr = query(simulationId, null, time, time,
    // keywords).result.toString();
    // processATime(expected_output_series, simulationId, responseStr,
    // testResultSeries, index+"");
    // index++;
    // }
    //
    // System.out.println("TestManager number of conflicts: "+ " total " +
    // testResultSeries.getTotal());
    // return testResultSeries;
    // }
    ...
    // public void processATime(JsonObject expected_output_series, String
    // simulationId, String responseStr, TestResultSeries testResultSeries, String
    // index) {
    ...
    // JsonArray points_array =
    // meas.getAsJsonObject().get("points").getAsJsonArray();
    // for (JsonElement point : points_array) {
    // JsonArray entry_array =
    // point.getAsJsonObject().get("row").getAsJsonObject().get("entry").getAsJsonArray();
    //// System.out.println(" ");
    ...
    // }
    //
    // JsonObject outputObject = buildOutputObject(simulationId, simOutputObject,
    // time, measurements);
    // System.out.println(simOutputObject.toString());
    // TestResults tr = compareResults.compareExpectedWithSimulationOutput(index+"",
    // outputObject, expected_output_series);
    // if (tr != null) {
    ...
    public TestResultSeries processWithAllTimes(String simulationId, TestConfig testConfig, String responseOne,
            String responseTwo) {
        TestResultSeries testResultSeries = new TestResultSeries();
        ...
        for (Entry<String, JsonElement> time_entry : simOutputObjectOne.entrySet()) {
            ...
            TestResults tr = compareResults.compareExpectedWithSimulationOutput(time_entry.getKey(),
                    time_entry.getValue().getAsJsonObject(), simOutputObjectTwo);
            // tr.pprint();
            if (tr != null) {
        ...
        // System.out.println("Index: " + index + " TestManager number of conflicts: "+
        ...
        if (simInputObject.entrySet().isEmpty() || expected_input_series.entrySet().isEmpty()) {
            ...
            for (Entry<String, JsonElement> time_entry : simInputObject.entrySet()) {
                System.out.println(time_entry);
                TestResults tr = compareResults.compareExpectedWithSimulationInput(time_entry.getKey().toString(),
                        time_entry.getKey().toString(), simInputObject, expected_input_series);
                if (tr != null) {
                    testResultSeries.add(time_entry.getKey(), time_entry.getKey(), tr);
                }
            }
        ...
        // rebaseAndCompare(testResultSeries, compareResults, simInputObject,
    ...
    public void rebaseAndCompare(TestResultSeries testResultSeries, CompareResults compareResults,
        ...
        for (Entry<Integer, Integer> time_entry : newKeys1.entrySet()) {
            ...
            if (simInputObject.has(timeOne)) {
                TestResults tr = compareResults.compareExpectedWithSimulationInput(timeOne, timeTwo, simInputObject,
                        expected_input_series);
                if (tr != null) {
    ...
    public TestResultSeries processWithAllTimes(JsonObject expected_series, TestConfig testConfig, String response) {
        ...
        if (expectedObject.has("output") && expected_series.has("output")) {
            ...
            for (Entry<String, JsonElement> time_entry : simOutputObject.entrySet()) {
                // System.out.println(time_entry);
                TestResults tr = compareResults.compareExpectedWithSimulationOutput(time_entry.getKey(),
                        time_entry.getValue().getAsJsonObject(), expected_output_series);
                if (tr != null) {
                    testResultSeries.add(time_entry.getKey(), time_entry.getKey(), tr);
                }
                // index++;
    ...
    public HashMap<Integer, Integer> rebaseKeys(JsonObject simInputObject, JsonObject expected_input_series) {
        ...
        // inputKeys1.add(1590612488); inputKeys1.add(1590612503);
        // inputKeys1.add(1590612518);
        // inputKeys2.add(1590616253); inputKeys2.add(1590616268);
        // inputKeys2.add(1590616283);
    ...
    /**
     * Create a map to match the match the indexes of the input keys. The mapping is
     * created by using the differences between the values of the set of inputKeys2
     * and the first value of the inputKeys1 to keys a map that is based on the
     * difference between the times instead of using exact matches.
     *
     * @param inputKeys1
     * @param inputKeys2
     * @return
    ...
    public JsonObject buildExpectedFromTimeseries(JsonArray measurements) {
        ...
        for (JsonElement measurement : measurements) {
            ...
            if (measurement.getAsJsonObject().get("hasSimulationMessageType").getAsString().equals("OUTPUT")) {
                ...
                simOutputObject.get(time).getAsJsonObject().get("message").getAsJsonObject().get("measurements")
                        .getAsJsonArray().add(measurement);
            } else { // INPUT
                ...
                measurement.getAsJsonObject().remove("simulation_id");
                measurement.getAsJsonObject().remove("time");
                simInputObject.get(time).getAsJsonObject().get("message").getAsJsonObject().get("measurements")
                        .getAsJsonArray().add(measurement);
            }
    ...
    // public String getListOfTime(String simulationId, JsonObject
    // expected_output_series){
    // CompareResults compareResults = new CompareResults();
    // Map<String, JsonElement> expectedOutputMap =
    // compareResults.getExpectedOutputMap("0", expected_output_series);
    // Set<String> keySet = expectedOutputMap.keySet();
    //// return
    // timeSeriesQuery(simulationId,"_e10b535c-79f3-498b-a38f-11d1cc50f3a0",
    // null,null);
    // for (String mrid : keySet) {
    //// String response = query(simulationId, mrid,
    // null,null,null).result.toString();
    ...
    // public String getListOfTime(String simulationId, String
    // expected_output_series){
    // CompareResults compareResults = new CompareResults();
    // Map<String, JsonElement> expectedOutputMap =
    // compareResults.getExpectedOutputMap("0", expected_output_series);
    ...
    // for (JsonElement point : points_array) {
    // JsonArray entry_array =
    // point.getAsJsonObject().get("row").getAsJsonObject().get("entry")
    // .getAsJsonArray();
    // for (JsonElement entry : entry_array) {
    // String key = entry.getAsJsonObject().get("key").getAsString();
    // if (key.equals("time")) {

...
public class BGPowergridModelDataManagerImpl implements PowergridModelDataManager {
    ...
    public BGPowergridModelDataManagerImpl() {
        ...
        // queryHandler = new
        // BlazegraphQueryHandler(configManager.getConfigurationProperty(GridAppsDConstants.BLAZEGRAPH_HOST_PATH));
    ...
    public static void main(String[] args) {
        ...
        // BGPowergridModelDataManagerImpl bg = new
        // BGPowergridModelDataManagerImpl("http://localhost:9999/blazegraph/namespace/kb/sparql");
        // BGPowergridModelDataManagerImpl bg = new
        // BGPowergridModelDataManagerImpl("http://192.168.99.100:8889/bigdata/namespace/kb/sparql");
        ...
        try {
            // String query = "select ?s ?p ?o where {?s r:type c:ConnectivityNode. ?s ?p
            // ?o}";
            // System.out.println(bg.query("ieee13", query, "JSON"));

            // bg.queryObject("ieee13", "_211AEE43-D357-463C-95B9-184942ABE3E5", "JSON");
            // System.out.println(bg.queryObjectTypes("_4F76A5F9-271D-9EB8-5E31-AA362D86F2C3",
            // "JSON", "12345", "user"));
            ...
            // String model = bg.queryModel("4F76A5F9-271D-9EB8-5E31-AA362D86F2C3", "", "",
            // "XML", "12345", "user");
            // String model = bg.queryModel("503D6E20-F499-4CC7-8051-971E23D0BF79", "", "",
            // "XML", "12345", "user");
            ...
            // System.out.println(bg.queryModel("_4F76A5F9-271D-9EB8-5E31-AA362D86F2C3", "",
            // "", "XML", "12345", "user"));
            // System.out.println(bg.queryModelNames("XML"));
            // System.out.println(bg.queryModelNamesAndIds("XML", "12345", "user"));

            // System.out.println(bg.queryObjectIds("JSON",
            // "4F76A5F9-271D-9EB8-5E31-AA362D86F2C3", "LoadBreakSwitch", "12345", "user"));
            // test with both object id and type
            // System.out.println(bg.queryObjectDictByType("JSON",
            // "C1C3E687-6FFD-C753-582B-632A27E28507", "LinearShuntCompensator",
            // "_EF2FF8C1-A6A6-4771-ADDD-A371AD929D5B", "12345", "user")); //ieee123
            // test with only object id
            // System.out.println(bg.queryObjectDictByType("JSON",
            // "C1C3E687-6FFD-C753-582B-632A27E28507", null,
            // "_EF2FF8C1-A6A6-4771-ADDD-A371AD929D5B", "12345", "user")); //ieee123
            // test with only object type
            // System.out.println(bg.queryObjectDictByType("JSON",
            // "C1C3E687-6FFD-C753-582B-632A27E28507", "LinearShuntCompensator", null,
            // "12345", "user")); //ieee123
            // test with neither object or type, should fail
            // try{
            // System.out.println(bg.queryObjectDictByType("JSON",
            // "C1C3E687-6FFD-C753-582B-632A27E28507", null, null, "12345", "user"));
            // //ieee123
            // }catch (Exception e) {
            ...
            // }
            // System.out.println(bg.queryObjectDictByType("JSON",
            // "4F76A5F9-271D-9EB8-5E31-AA362D86F2C3", "LinearShuntCompensator", null,
            // "12345", "user")); //ieee8500
            // System.out.println(bg.queryMeasurementDictByObject("JSON",
            // "4F76A5F9-271D-9EB8-5E31-AA362D86F2C3", null,
            // "_7A02B3B0-2746-EB24-45A5-C3FBA8ACB88E", "12345", "user"));
            // System.out.println(bg.queryMeasurementDictByObject("JSON",
            // "4F76A5F9-271D-9EB8-5E31-AA362D86F2C3", "LinearShuntCompensator", null,
            // "12345", "user"));
    ...
    @Override
    public String query(String modelId, String query, String resultFormat, String processId, String username)
            throws Exception {
        ...
        if (resultFormat.equals(ResultFormat.JSON.toString())) {
            ...
            // logManager.log(new LogMessage(processId, timestamp, "Result Format not
            // recognized, '"+resultFormat+"'", LogMessage.LogLevel.ERROR,
            // LogMessage.ProcessStatus.ERROR, storeToDb), username);
        }
    ...
    @Override
    public String queryObject(String modelId, String mrid, String resultFormat, String processId, String username)
            throws Exception {
        ...
        if (resultFormat.equals(ResultFormat.JSON.toString())) {
            ...
            // logManager.log(new LogMessage(processId, timestamp, logMessage, logLevel,
            // processStatus, storeToDb), username);
        }
    ...
    public ResultSet queryObjectResultSet(String modelId, String mrid, String processId, String username) {
        ...
        BlazegraphQueryHandler queryHandler = new BlazegraphQueryHandler(getEndpointURL(modelId), logManager, processId,
                username);
    ...
    @Override
    public String queryModel(String modelId, String objectType, String filter, String resultFormat, String processId,
        ...
        while (rs.hasNext()) {
            ...
            if (!alreadySeen.contains(subjectUri) && !newIds.contains(subjectUri) && !RDF_TYPE.equals(propertyName)
                    && subjectUri.startsWith(baseUrl)) {
                newIds.add(subjectUri);

            }
        }

        // rs = queryModelResultSet(modelId, objectType, filter, processId, username,
        // true);

        // Tracks which subjects have been seen already and follows links to pull
        // information on those that haven't been included yet
        while (rs.hasNext() || newIds.size() > 0) {
            while (rs.hasNext()) {
                ...
                if (qs.get(OBJECT).isLiteral()) {
                    ...
                    if (!alreadySeen.contains(value) && !newIds.contains(value) && value.startsWith(baseUrl)
                            && !feederProperty.equals(propertyName)) {
                        newIds.add(value);
                    }
                }
            ...
            if (newIds.size() > 0) {
                ...
                while (rs.hasNext()) {
                    ...
                    if (!alreadySeen.contains(subjectUri) && !newIds.contains(subjectUri)
                            && !RDF_TYPE.equals(propertyName) && !feederProperty.equals(propertyName)
                            && subjectUri.startsWith(baseUrl)) {
                        newIds.add(subjectUri);
                    }
                }
    ...
    @Override
    public ResultSet queryModelResultSet(String modelId, String objectType, String filter, String processId,
            String username) {
        return queryModelResultSet(modelId, objectType, filter, processId, username, false);
    }

    protected ResultSet queryModelResultSet(String modelId, String objectType, String filter, String processId,
            String username, boolean multiLevel) {
        if (modelId == null) {
    ...
    @Override
    public String queryObjectIds(String resultFormat, String modelId, String objectType, String processId,
            String username) {
        return formatStringList(queryObjectIdsList(modelId, objectType, processId, username), "objectIds",
                resultFormat);
    ...
    @Override
    public String queryObjectDictByType(String resultFormat, String modelId, String objectType, String objectId,
    ...
    public ResultSet queryObjectDictByTypeResultSet(String modelId, String objectType,
        ...
        if ((objectType == null || objectType.trim().length() == 0)
                && (objectId == null || objectId.trim().length() == 0)) {
            throw new RuntimeException(
                    "queryObjectDict: both object id and object type missing, at least one required");
        }
        ...
        if ((objectId == null || objectId.trim().length() == 0) && objectType != null
    ...
    @Override
    public String queryMeasurementDictByObject(String resultFormat, String modelId, String objectType, String objectId,
            String processId, String username) throws Exception {
        String result = null;
    ...
    @Override
    public ResultSet queryMeasurementDictByObjectResultSet(String modelId, String objectType, String objectId,
    ...
    // protected void sendResult(String result, String resultTopic) throws
    // Exception{
    ...
    protected boolean isValidURI(String url) {
        ...
        try {
            // Use URI.create() instead of deprecated URL(String) constructor
    ...
    public String queryModelNamesAndIds(String resultFormat, String processId, String username) {
        ...
        if (resultFormat.equals(ResultFormat.JSON.toString())) {
            ...
            while (rs.hasNext()) {
                ...
                String value = feederName + "|" + feederId + "|" + stationName + "|" + stationId + "|" + subregionName
                        + "|" + subRegionId + "|" + regionName + "|" + regionId;
                values.add(value);
            }
            return String.join(",", values);
        } else {
    ...
    public ResultSet queryModelNamesAndIdsResultSet(String processId, String username) {
        String modelNameQuery = "SELECT ?" + FEEDER_NAME + " ?" + FEEDER_ID + " ?" + STATION_NAME + " ?" +
                STATION_ID + " ?" + SUBREGION_NAME + " ?" + SUBREGION_ID + " ?" + REGION_NAME + " ?" + REGION_ID
                + " WHERE {"
                + " ?s r:type c:Feeder."

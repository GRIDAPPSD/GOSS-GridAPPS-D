...
public class BGPowergridModelDataManagerHandlerImpl implements DataManagerHandler {
    ...
    public Serializable handle(Serializable requestContent, String processId, String username) throws Exception {
        ...
        if (PowergridModelDataRequest.RequestType.QUERY.toString().equals(pgDataRequest.requestType)) {
            ...
            return dataManager.query(pgDataRequest.getModelId(), pgDataRequest.getQueryString(),
                    pgDataRequest.getResultFormat(), processId, username);
            ...
            return dataManager.queryModel(pgDataRequest.getModelId(), pgDataRequest.getObjectType(),
                    pgDataRequest.getFilter(), pgDataRequest.getResultFormat(), processId, username);
        } else if (PowergridModelDataRequest.RequestType.QUERY_MODEL_NAMES.toString()
                .equals(pgDataRequest.requestType)) {
        ...
        } else if (PowergridModelDataRequest.RequestType.QUERY_MODEL_INFO.toString()
                .equals(pgDataRequest.requestType)) {
            if (!verifyResultFormat(pgDataRequest.getResultFormat())) {
                // TODO send error
            }
            ...
            if (pgDataRequest.getModelId() == null || pgDataRequest.getObjectId() == null
                    || !verifyResultFormat(pgDataRequest.getResultFormat())) {
                // TODO send error
            }
            return dataManager.queryObject(pgDataRequest.getModelId(), pgDataRequest.getObjectId(),
                    pgDataRequest.getResultFormat(), processId, username);
        } else if (PowergridModelDataRequest.RequestType.QUERY_OBJECT_TYPES.toString()
                .equals(pgDataRequest.requestType)) {
            if (pgDataRequest.getModelId() == null || !verifyResultFormat(pgDataRequest.getResultFormat())) {
                // TODO send error
            }
            return dataManager.queryObjectTypes(pgDataRequest.getModelId(), pgDataRequest.getResultFormat(), processId,
                    username);
        } else if (PowergridModelDataRequest.RequestType.QUERY_OBJECT_IDS.toString()
                .equals(pgDataRequest.requestType)) {
            if (pgDataRequest.getModelId() == null || !verifyResultFormat(pgDataRequest.getResultFormat())) {
                // TODO send error
            }
            return dataManager.queryObjectIds(pgDataRequest.getResultFormat(), pgDataRequest.getModelId(),
                    pgDataRequest.getObjectType(), processId, username);
        } else if (PowergridModelDataRequest.RequestType.QUERY_OBJECT_DICT.toString()
                .equals(pgDataRequest.requestType)) {
            if (pgDataRequest.getModelId() == null || !verifyResultFormat(pgDataRequest.getResultFormat())) {
                // TODO send error
            }
            return dataManager.queryObjectDictByType(pgDataRequest.getResultFormat(), pgDataRequest.getModelId(),
                    pgDataRequest.getObjectType(), pgDataRequest.getObjectId(), processId, username);
        } else if (PowergridModelDataRequest.RequestType.QUERY_OBJECT_MEASUREMENTS.toString()
                .equals(pgDataRequest.requestType)) {
            if (pgDataRequest.getModelId() == null || !verifyResultFormat(pgDataRequest.getResultFormat())) {
                // TODO send error
            }
            return dataManager.queryMeasurementDictByObject(pgDataRequest.getResultFormat(), pgDataRequest.getModelId(),
                    pgDataRequest.getObjectType(), pgDataRequest.getObjectId(), processId, username);
        } else {
    ...
    boolean verifyResultFormat(String resultFormat) {
        return resultFormat != null
                && (resultFormat.equals(ResultFormat.JSON) || resultFormat.equals(ResultFormat.XML));
    }
}
